...
@section('content')
<div class="container">
    <h2 class="mb-4">Daftar Insiden berdasarkan Dinas</h2>
    ...
    <div class="card mb-4 shadow-sm">
        <div class="card-body">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-4 col-lg-2">
                    <label for="call_type" class="form-label visually-hidden">Jenis Panggilan</label>
                    <select id="call_type" class="form-select">
                        <option value="">Semua Jenis Panggilan</option>
                        <option value="1">Darurat</option>
                        <option value="2">Non-Darurat</option>
                    </select>
                </div>
                <div class="col-md-4 col-lg-2">
                    <label for="channel_id" class="form-label visually-hidden">Saluran</label>
                    <select id="channel_id" class="form-select">
                        <option value="">Semua Saluran</option>
                        <option value="1">Call Center</option>
                        <option value="2">WhatsApp</option>
                        <option value="3">Aplikasi Seluler</option>
                    </select>
                </div>
                <div class="col-md-4 col-lg-2">
...
<script>
    ...
    let currentPage = 1;
    // Ensure dinasParam is correctly retrieved. Using `location.pathname` is a good approach.
    ...
    function fetchData(page = 1) {
        ...
        const params = {
            ...
            call_type: getEl('call_type').value,
            channel_id: getEl('channel_id').value,
        };

        getEl('loading').style.display = 'block';
        getEl('incidentCards').innerHTML = ''; // Clear previous results

        axios.get(`/incident/by-dinas/${dinasParam}/list`, { params })
            .then(response => {
    ...
    function renderPagination(data) {
        ...
        if (totalPages <= maxPagesToShow) {
            // total pages less than max so show all
            startPage = 1;
            endPage = totalPages;
        } else {
    ...
    // Status mapping for labels and colors
    const statusMap = {
