...
@section('content')
    <div class="container">
        <h2 class="mb-4">Daftar Insiden berdasarkan Status
            {{ $status == 1 ? 'Baru' : ($status == 2 ? 'Proses' : 'Selesai') }}</h2>
...
@push('scripts')
    ...
    <script>
        ...
        function fetchData(page = 1) {
            ...
            axios.get(`/incident/by-status/${dinasParam}/list`, {
                    params
                })
                .then(response => {
        ...
        function resetFilter() {
            ['search', 'start_date', 'end_date', 'category', 'status', 'district', 'subdistrict', 'call_type', 'channel_id']
            .forEach(id => {
                getEl(id).value = '';
            });
